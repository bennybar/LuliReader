...
class LocalRssService {
  ...
  Future<void> _syncFeed(
    ...
    try {
      ...
      final articles = await _fetchWithRetry(feed, accountId, preDate, onProgress);
  ...
  Future<List<Article>> _fetchWithRetry(
    Feed feed,
    int accountId,
    DateTime preDate,
    void Function(String)? onProgress,
    ...
    for (int i = 0; i < attempts; i++) {
      try {
        return await _rssHelper.queryRssXml(feed, accountId, null, preDate);
      } catch (e) {
        onProgress?.call('  → Fetch failed (${i + 1}/$attempts): $e');
